...
int verifica(def_arvore arvore, int v)
{
    if (arvore==NULL) return 0;

    if (arvore->info == v) return 1;

    return( verifica(arvore->esquerdo,v)||verifica(arvore->direito,v));	
}
