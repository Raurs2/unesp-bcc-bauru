...
int conta_nos_maiores_que_X(def_arvore arvore,int X){
  int i,cont=0,cont1;
    
  	if(arvore==NULL) return 0;
    if(arvore->info>X) cont++;
  	for(i=0;i<Grau;i++){
  		 if(arvore->filhos[i]!=NULL) 
		    cont+=conta_nos_maiores_que_X(arvore->filhos[i],X); }
   return(cont);
}
...
int main(){
  def_arvore arvore;
   ...
   printf("\nPossui %d nos maiores que 5",conta_nos_maiores_que_X(arvore,5));
