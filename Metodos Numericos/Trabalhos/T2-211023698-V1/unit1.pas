...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure RadioButtonQualquerChange(Sender: TObject);
...
begin
   ...
   begin
     ...
     try
       ...
       ShowMessage('Valor de eps incorreto');
       Edit8.SetFocus;
       Exit;
     end;
     if (eps-(b-a)/20) > 1E-10 then
     begin
       ShowMessage('O máximo admissível para eps é (b-a)/20.');
       Edit4.SetFocus;
       Exit;
     end;
     Result := True;
...
begin
   ...
   FxR1(f, p, f_p);
   tot_it := 1;
   Label9.Caption := IntToStr(tot_it);
   Memo1.Lines.Add(IntToStr(tot_it)+#09+FloatToStr(p)+#09+FloatToStr(f_p)+#09#09#09+FloatToStr(a)+#09+FloatToStr(b));
   while (abs(a-b) > eps) and (abs(f_p) > eps) do
      begin
         FxR1(f, a, f_a);
         if (f_a*f_p > 0) then
            a := p
...
begin
   ...
   FxR1(f, p, f_p);
   Memo1.Clear;
   Memo1.Lines.Add('n_it'+#09+'p'+#09#09+'f(p)'+#09#09#09+'a'+#09#09+'b');
   Memo1.Lines.Add(IntToStr(n_it)+#09+FloatToStr(p)+#09#09+FloatToStr(f_p)+#09#09#09+
   ...
   f_q := 999999;
   while (f_p <> 0) and (abs(f_p - f_q) > eps) and (abs(a-b) > eps) do
   begin
     Application.ProcessMessages;
     if Interrompe then
     begin
...
begin
   ...
   repeat
      ...
      p1 := p0 - (f_p0/f1_p0);
      Label9.Caption := IntToStr(tot_it);
      Memo1.Lines.Add(IntToStr(tot_it)+#09#09+FloatToStr(p0)+#09#09+FloatToStr(p1));
   until (abs(p1 - p0) < eps) or (tot_it > max_it);
   Raiz := p1;
   FxR1(f, Raiz, f_Raiz);
   Memo1.Lines.Add('Resultado: x = '+FloatToStr(Raiz)+'; f(x) = '+FloatToStr(f_Raiz));
   Edit9.Text := FloatToStr(Raiz);
...
var
   tot_it, max_it, div_resto: Integer;
   p0, p1, f_p0, f1_p0: Extended;
begin
   Interrompe := False;
   Memo1.Clear;
...
procedure TForm1.RadioButtonQualquerChange(Sender: TObject);
begin
