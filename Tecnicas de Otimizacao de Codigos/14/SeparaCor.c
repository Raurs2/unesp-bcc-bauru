...
void processa(struct Pixel img[ALTU_IMG][LARG_IMG],
  ...
  struct Pixel corEtiqueta1 = {COR1};
  struct Pixel pixelPreto = {PRETO};
  int i, j;
  for (i = 0; i < ALTU_IMG; i++) {
    for (j = 0; j < LARG_IMG; j += 1) {
      struct Pixel pixelAtual = img[i][j];
      int dr = corEtiqueta1.r - pixelAtual.r;
      int dg = corEtiqueta1.g - pixelAtual.g;
      int db = corEtiqueta1.b - pixelAtual.b;
      if (sqrt(pow(dr, 2) + pow(dg, 2) + pow(db, 2)) < DIST_COR1) {
