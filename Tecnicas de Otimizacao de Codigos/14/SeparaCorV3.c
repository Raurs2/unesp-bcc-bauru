...
#define SATURA(x) x > 261120 ? 255 : (x < 0 ? 0 : x / 1024);
  ...
  for (i = 0; i < ALTU_IMG; i++) {
    for (j = 0; j < LARG_IMG; j += 3) {
      ...
      if (dr * dr + dg * dg + db * db < DIST_COR1 * DIST_COR1) {
        pixelAtual.r = SATURA(pixelAtual.r * 1228);
        pixelAtual.g = SATURA(pixelAtual.g * 1228);
        pixelAtual.b = SATURA(pixelAtual.b * 1228);
        imgSai[i][j] = pixelAtual;
      } else {
        pixelAtual.r = SATURA(pixelAtual.r * 307);
        pixelAtual.g = SATURA(pixelAtual.g * 307);
        pixelAtual.b = SATURA(pixelAtual.b * 307);
        imgSai[i][j] = pixelAtual;
      }
