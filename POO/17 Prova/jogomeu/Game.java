...
class Game extends JFrame {
    ...
    // Classe que adicionara as imagens no jogo
    class GameMap extends JPanel {
        GameMap() {
            // zerando trajetorias e geranod velocidade e localização aleatoria para os
            // meteoros
            for (int i = 0; i < 4; i++) {
    ...
    Game() {
        ...
        t = new Timer(1000, new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                score++;
                for (int i = 0; i < 4; i++) {
                    // faz os meteoros voltarem a cair com velocidade aumentanda e de posicoes
                    // aleatorias
                    if (meteorTrajectory[i] >= height) {
                        ...
                        // testa se os meteoros bateram nos jogadores, se baterem o jogo acaba e mostra
                        // a pontuação
                        if (getBounds(rand[i], meteorTrajectory[i], mWidth[i], mHeight[i])
                                .intersects(getBounds(p1x, py[0], pWidth[0], pHeight[0]))) {
                            JOptionPane.showMessageDialog(null, "MARCOU " + score + " PONTOS ANTES DE MORRER");
                            System.exit(1);
                        } else if (getBounds(rand[i], meteorTrajectory[i], mWidth[i], mHeight[i])
                                .intersects(getBounds(p2x, py[1], pWidth[1], pHeight[1]))) {
                            JOptionPane.showMessageDialog(null, "MARCOU " + score + " PONTOS ANTES DE MORRER");
                            System.exit(1);
                        }
                    }
